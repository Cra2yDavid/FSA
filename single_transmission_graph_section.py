...
class TransmissionSectionEnv(gym.Env):
    ...
    def __init__(self, args, evaluation=False):
        ...
        self.graph_u = dgl.from_networkx(nxgraph)   # undirected graph
        u_list, v_list = [], []
        ...
        self.graph_d = dgl.graph((torch.tensor(u_list), torch.tensor(v_list)))  # directed graph
        ...
        if self.env_id == 'S10case9241' or self.env_id == 'S4case9241':
            ...
            self.n_line = len(self.section_set)
    ...
    def _get_state(self):
        ...
        if self.env_id == 'S10case9241' or self.env_id == 'S4case9241':
            ...
            p_dv = p_line.loc[self.section_set].to_list()
            q_dv = q_line.loc[self.section_set].to_list()
            v_dv = v_line.loc[self.section_set].to_list()
            t_dv = t_line.loc[self.section_set].to_list()

            assert len(p_dv) == self.n_line, False

            p_dv = torch.nn.functional.normalize(torch.tensor(p_dv, dtype=torch.float32), p=2, dim=-1)
            q_dv = torch.nn.functional.normalize(torch.tensor(q_dv, dtype=torch.float32), p=2, dim=-1)
            v_dv = torch.nn.functional.normalize(torch.tensor(v_dv, dtype=torch.float32), p=2, dim=-1)
            t_dv = torch.nn.functional.normalize(torch.tensor(t_dv, dtype=torch.float32), p=2, dim=-1)
            ...
            p_dv = torch.cat((torch.tensor(p_line, dtype=torch.float32), torch.tensor(p_trafo, dtype=torch.float32)))
            q_dv = torch.cat((torch.tensor(q_line, dtype=torch.float32), torch.tensor(q_trafo, dtype=torch.float32)))
            v_dv = torch.cat((torch.tensor(v_line, dtype=torch.float32), torch.tensor(v_trafo, dtype=torch.float32)))
            t_dv = torch.cat((torch.tensor(t_line, dtype=torch.float32), torch.tensor(t_trafo, dtype=torch.float32)))
