...
class SelfAttentionNetWeighted_Factor(nn.Module):
    def __init__(self, output_shape,
        ...
        self.alpha = nn.Parameter(torch.Tensor([1 / self.task_num * 4 for _ in range(task_num * 4)]),
                                  requires_grad=True)
        ...
        if self.use_dueling:  # dueling DQN
            ...
            assert False, 'FSA must use dueling DQN architecture.'
    ...
    def forward(self, x, state=None, show_grid=False, info=None):
        ...
        q = self.w_q(torch.cat((embedding_task), dim=1))
        self.alpha.data = self.f0(self.alpha)
        q = torch.matmul(self.alpha, q).reshape(ba, 1, -1)
        ...
        out = v_p * attn[:, 0, :].unsqueeze(2) + v_q * attn[:, 1, :].unsqueeze(2) + v_v * attn[:, 2, :].unsqueeze(
            2) + v_t * attn[:, 3, :].unsqueeze(2)

        out0 = torch.mean(out, dim=1)

        if self.use_dueling:  # Dueling DQN
            Q, V = self.Q(out0), self.V(out0)
            out = Q - Q.mean(dim=1, keepdim=True) + V
        else:
            assert False, 'FSA must use dueling DQN architecture.'
        if show_grid:
            return out, state, out0.detach()
        else:
            return out, state


# FSA-Factor-O
class SelfAttentionNetWeighted_Factor_O(nn.Module):
    def __init__(self, output_shape,
                 em_input_shape, state_input_shape,
                 task_num: int,
                 hidden_type='MLP',
    ...
    def forward(self, x, state=None, show_grid=False, info=None):
        ...
        if self.hidden_type in ['GIN_Factor', 'GCN_Factor']:
            ...
            # p value convolution: Active power
            k_p = self.w_k_p(bg, state_in[:, 0].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)
            v_p = self.w_k_p(bg, state_in[:, 0].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)
            # q value convolution: Reactive power
            k_q = self.w_k_q(bg, state_in[:, 1].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)
            v_q = self.w_k_q(bg, state_in[:, 1].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)
            # v value convolution: Voltage
            k_v = self.w_k_v(bg, state_in[:, 2].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)
            v_v = self.w_k_v(bg, state_in[:, 2].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)
            # t value convolution: Phase angle
            k_t = self.w_k_t(bg, state_in[:, 3].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)
            v_t = self.w_k_t(bg, state_in[:, 3].reshape(-1, 1)).reshape(-1, bus_num, self.dim_represent)

        else:
            k_p = self.w_k_p(state_in[:, 0].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
            v_p = self.w_k_p(state_in[:, 0].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
            # q value convolution: Reactive power
            k_q = self.w_k_q(state_in[:, 1].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
            v_q = self.w_k_q(state_in[:, 1].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
            # v value convolution: Voltage
            k_v = self.w_k_v(state_in[:, 2].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
            v_v = self.w_k_v(state_in[:, 2].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
            # t value convolution: Phase angle
            k_t = self.w_k_t(state_in[:, 3].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
            v_t = self.w_k_t(state_in[:, 3].reshape(-1, 1)).reshape(ba, -1, self.dim_represent)
...
# FSA-Factor-S
class SelfAttentionNetWeighted_Factor_S(nn.Module):
    def __init__(self, output_shape,
                 em_input_shape, state_input_shape,
                 task_num: int,
                 hidden_type='MLP',
...
# FSA-Factor-E
class SelfAttentionNetWeighted_Factor_E(nn.Module):
    def __init__(self, output_shape,
                 em_input_shape, state_input_shape,
                 task_num: int,
                 hidden_type='MLP',
    ...
    def forward(self, x, state=None, show_grid=False, info=None):
        ...
        if self.hidden_type in ['GIN_Factor', 'GCN_Factor']:
            ...
            state_in = state_in.reshape(-1, 4)
            # p value convolution: Active power
            v_p = self.w_v(bg, state_in.reshape(-1, 4)).reshape(-1, bus_num, self.dim_represent)
